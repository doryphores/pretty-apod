...
urlpatterns = patterns('apod.views',
	...
	url(r'^tags/(?P<tag>[\w\-]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/$', 'image', name='tag_image'),
	url(r'^tags/(?P<tag>[\w\-]+)/(?P<year>\d{4})/(?P<month>\d{1,2})/$', 'month', name='tag_month'),
	url(r'^tags/(?P<tag>[\w\-]+)/(?P<year>\d{4})/$', 'year', name='tag_year'),
